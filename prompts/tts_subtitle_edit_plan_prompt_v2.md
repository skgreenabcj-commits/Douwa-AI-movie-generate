...
## `<sub>` の2種類の用途と `</sub>` 直後の句読点ルール（重要）

`<sub>` タグには用途が2種類あり、`</sub>` 直後の `、` `。` の扱いが異なる。

### 用途A：アクセント補助（alias = text が同一）

擬音語・擬態語など、読みは正しいがアクセントパターンが崩れやすい語に使用する。
この場合、`</sub>` 直後の `、` `。` は TTS がアーティファクト（"あぁ" 挿入・"テン" 読み等）を生成する。
**必ず `<break>` タグに置換すること。**

```xml
<!-- ❌ NG: アクセント補助 + </sub> 直後の読点 → "あぁ" アーティファクト -->
猿が<sub alias="ひょっこり">ひょっこり</sub>、顔を出しました
<sub alias="どんぶらこ">どんぶらこ</sub>、<sub alias="どんぶらこ">どんぶらこ</sub>と、

<!-- ✅ OK: </sub> 直後の句読点を <break> に置換 -->
猿が<sub alias="ひょっこり">ひょっこり</sub><break time="200ms"/>顔を出しました
...
### 用途B：読み補助（alias ≠ text、漢字の読みを固定する）

多音字など TTS が誤読みする語に使用する。

#### 文節拡張ルール（重要）

TTS エンジンは `</sub>` 閉じタグ直後に micro-break を挿入する性質がある。
対象語だけをラップすると文中に不自然な「間」が生じるため、
**誤読語を含む文節から直後の読点「、」または句点「。」直前まで延伸してラップすること。**

- 目安: 誤読語 + 直後の述語や補語も含めて、次の句読点の直前で `</sub>` を閉じる
- `</sub>` 直後の `、` `。` は**そのまま維持する**（`<break>` への置換は不要）

```xml
<!-- ❌ NG（単語スコープ）: "宝物" の手前で micro-break が発生 -->
棚の<sub alias="なかに">中に</sub>宝物があった。

<!-- ✅ OK（文節拡張）: break は句点前の自然な位置に収まる -->
棚の<sub alias="なかにたからものがあった">中に宝物があった</sub>。

<!-- ❌ NG（単語スコープ） -->
お<sub alias="なかに">中に</sub>入ってください。

<!-- ✅ OK（文節拡張） -->
お<sub alias="なかにはいってください">中に入ってください</sub>。

<!-- ✅ 確認済み誤読み例: お供（おきょうと誤読） -->
<sub alias="おともさせてください">お供させてください</sub>。
<sub alias="おともなかまにくわわりました">お供、仲間に加わりました</sub>。
```
...
`<sub>` による読み補助は、**TTS が誤読みすることが確認された語のみ**に限定すること。
通常の漢字（大きな・流れて・昔々 等）は TTS が正しく読めるため `<sub>` は不要。

下記「読み違い辞書」に記載の語が出現した場合は、文節拡張ルールを適用して補正すること。

> **判断基準**: 単に多音字であるだけでは `<sub>` を付与しない。
> 「この語を TTS が誤読みした」という実績があるか、明らかに誤読みリスクが高い場合のみ使用する。
...
10. **読み補助の文節拡張**: 読み違い辞書に記載の語が出現した場合、対象語単体ではなく次の句読点直前まで `<sub alias>` スコープを延伸すること
