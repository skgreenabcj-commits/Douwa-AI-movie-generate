...
この場合、`</sub>` 直後の `、` `。` は**そのまま維持する**（`<break>` に置換しない）。

```xml
<!-- ✅ OK: 読み補助では </sub> 直後の読点はそのまま -->
...
### 読み補助 `<sub>` の使用条件（確認済み誤読みのみ）

`<sub>` による読み補助は、**TTS が誤読みすることが確認された語のみ**に限定すること。
通常の漢字（大きな・流れて・昔々 等）は TTS が正しく読めるため `<sub>` は不要。

| ❌ そのまま（確認済み誤読み） | ✅ `<sub>` 対処後 | 誤読みパターン |
|---|---|---|
| `お供させてください` | `<sub alias="おとも">お供</sub>させてください` | `おきょう`（供述と混同） |
| `お供します` | `<sub alias="おとも">お供</sub>します` | 同上 |
