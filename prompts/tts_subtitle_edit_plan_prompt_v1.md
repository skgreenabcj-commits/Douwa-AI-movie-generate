...
## `</sub>` 直後の句読点は必ず `<break>` に置換（重要）

`</sub>` の直後にある `、` `。` は、後続が別の `<sub>` タグでも通常テキストでも、
TTS が予期しない音声アーティファクト（"テン" 読み上げ・息継ぎ音・"あぁ" 挿入等）を生成する。
**`</sub>` の直後に来る句読点はすべて `<break>` タグに置換すること。**

```xml
<!-- ❌ NG パターン1: </sub> 直後 + 通常テキスト → "あぁ" 等のアーティファクト -->
猿が<sub alias="ひょっこり">ひょっこり</sub>、顔を出しました

<!-- ❌ NG パターン2: </sub> 直後 + <sub> → "テン" と読まれる -->
<sub alias="どんぶらこ">どんぶらこ</sub>、<sub alias="どんぶらこ">どんぶらこ</sub>と、

<!-- ✅ OK: どちらのケースも </sub> 直後は <break> に置換 -->
...
| 置換対象 | 置換後 | 備考 |
|---|---|---|
| `</sub>、` | `</sub><break time="200ms"/>` | 後続が `<sub>` でも通常テキストでも同様 |
| `</sub>。` | `</sub><break time="300ms"/>` | 同上 |
...
### 多音字・誤読みやすい語への `<sub>` 必須対応（重要）

以下の語は TTS が誤読みすることが確認されている。必ず `<sub>` タグで読みを固定すること。

| ❌ そのまま（誤読みリスク） | ✅ `<sub>` 対処後 | 誤読みパターン |
|---|---|---|
| `お供させてください` | `<sub alias="おとも">お供</sub>させてください` | `おきょう`（供述と混同） |
| `お供します` | `<sub alias="おとも">お供</sub>します` | 同上 |

> **判断基準**: 訓読み・音読みが複数ある漢字（`供` `他` `大` 等）が文中に現れたら、
> 文脈から読みを確定し `<sub>` タグを付与すること。
