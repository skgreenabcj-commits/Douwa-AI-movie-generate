# STEP_07 Image Prompts Build — Prompt v2
...
- **character_refs**: このシーンに登場するキャラクターおよびコアアイテムの Visual Bible `key_name` を **正確な文字列で** 配列に列挙する
  - `INPUT_DATA.visual_bible` の `category = "character"` のエントリのうち、このシーンに実際に登場するものだけを選ぶ
  - **`category = "core_item"` のアイテムが、そのシーンで視覚的に存在する（手に持つ・描写される・画面に映る）場合も `key_name` を含める**
  - `key_name` の文字列は一字一句変えずにそのままコピーすること（例: `"桃太郎（5〜9歳・幼少期）"`, `"きびだんご"`）
  - 登場しないキャラクター・アイテムは含めない
  - 例: `["桃太郎（5〜9歳・幼少期）", "犬", "きびだんご"]`
...
10. `character_refs` には、このシーンに登場する `category="character"` の VB エントリの `key_name` を **変更せず** 列挙すること。加えて `category="core_item"` のアイテムがシーンに視覚的に存在する場合も同様に key_name を含めること
  ...
  "image_prompts": [
    {
      ...
      "character_refs": ["（登場キャラクターの VB key_name を正確な日本語文字列で列挙。core_item も登場する場合は含める）"],
...
桃太郎（PJT-001）シーン「桃太郎がきびだんごを犬に渡す」のサンプル出力（core_item 登場例）:

```json
{
  "image_prompts": [
    {
      "scene_record_id": "PJT-001-SCN-007",
      "character_refs": ["桃太郎（5〜9歳・幼少期）", "犬", "きびだんご"],
      "prompt_base": "Children's picture-book illustration, soft watercolor style, warm pastel tones, gentle ink outlines, 16:9 landscape, high resolution, 2K quality",
      "prompt_character": "Momotaro: warm smile, extending arm forward in a giving gesture; Dog: eyes wide with delight, tail raised, leaning forward eagerly",
      "prompt_scene": "Sunlit mountain path, small round yellow kibidango (millet dumplings) being offered between Momotaro and the Dog, green forest background with dappled light",
      "prompt_composition": "Medium two-shot, Momotaro left-center offering kibidango, Dog right-center receiving eagerly, both facing each other, warm afternoon sunlight from behind creating soft rim lighting",
      "negative_prompt": "dark tones, scary expressions, violence, blood, photorealistic, 3D render, fluorescent colors, neon colors, adult content, modern objects, no text, no letters, no captions, no subtitles, no story narration text, watermark, logo, blurry, low quality"
    }
  ]
}
