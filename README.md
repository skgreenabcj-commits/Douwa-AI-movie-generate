...
**背景**  
処理対象プロジェクトが増加するにつれ、STEP_04/05 が発行する Google Sheets Read リクエスト数が
`2 + 2N`（N = プロジェクト件数）に達し、無料枠の **300 req/min** を超過するエラーが発生した。

**変更内容**  
プロジェクトループ前に `spreadsheets.values.batchGet` で必要な全シートを 1 回の API コールで取得し、
ループ内はメモリフィルタのみで処理するよう改修した。

| 指標 | 改修前 | 改修後 |
|---|---|---|
| GSS Read 回数（N プロジェクト） | `2 + 2N` 回 | **1 回**（batchGet） |
| 20 件処理時 | 42 回 | **1 回** |
...
| ファイル | 追加関数 |
|---|---|
| `src/lib/sheets-client.ts` | `readSheetsBatch()` — batchGet ラッパー |
| `src/lib/load-project-input.ts` | `filterProjectsByIds()` — メモリフィルタ版 |
| `src/lib/load-scenes.ts` | `filterScenesByProjectId()` — メモリフィルタ版 |
| `src/lib/load-script.ts` | `filterFullScriptByProjectId()` — メモリフィルタ版 |
| `src/lib/load-runtime-config.ts` | `parseRuntimeConfig()` — rows → Map 変換 |
| `src/steps/step04-05-script-build.ts` | batchGet 一括取得 + filter 関数への差し替え |

既存関数（`readSheet`, `readProjectsByIds`, `loadScenesByProjectId` 等）は削除せず残存。  
他ステップへの影響はない。
