...
> - v1.2 (2026-04-04): クロスレビュー前の最終整合確認版。
>   §4.4 step 7 に short+full 依存性ルール（Fix #1）を明記。
>   §7.6 record_id 突合ルールを強化版（Fix #5: fail-fast・フォールバック上限）に更新。
>   §11 に short_use=Y 0件 SKIPPED ルール（Fix #6）を追記。
>   §13 subtitle_short_2 の統一ルール（Fix #4: required・空文字可）を §8/§9/§13 で明記。
>   §17 に partial success の詳細パターン（short_skipped / dependency_failure）を追加。
>   §18 に short_use=Y 0件 SKIPPED ケースを追加。
>   §20 のログ表に dependency_failure / short_skipped を追加。
>   §25 成果物一覧に write-app-log.ts ログビルダー追加分（Fix #7/#8）を反映。
>   §26 に Fix #1〜#9 の実装補足を追記。
...
7. Short生成時（STEP_04）:
   - **⚠️ `video_format=short+full` 時は Full の成功を前提とする（Fix #1）**
     - Full が失敗した場合は Short を実行しない（`dependency_failure`）
     - 理由: `short+full` モードの Short は Full script を参照して tuning する成果物であり、
       Full 参照がない状態では intended design を満たさない
   - `short_use=Y` の scene のみを AI 入力として渡す（`short_use=N` は行を生成しない）
   - `short_use=Y` が 0件の場合は SKIPPED 扱い（失敗ではない）
...
8. `00_Project` を最小更新（`current_step` は §17 の状態表に従う）
   ...
   - Full 失敗により Short が dependency_failure でスキップされた場合も `current_step` を更新しない
9. `100_App_Logs` に成功 / 失敗 / partial success / skipped を記録
...
- `04_Script_Full`
  - STEP_04 実行時、`video_format = short+full` **かつ Full Script が成功済みの場合**のみ参照
...
### 7.6 AI 出力の `record_id` 突合ルール（Fix #5 強化版）

- AI 出力の各要素の `record_id` と `02_Scenes.record_id` を突合する
- **件数不一致（AI出力件数 ≠ 入力 scenes 件数）は validation 段階で fail-fast**
  - `validateScriptFullAiResponse` / `validateScriptShortAiResponse` の `expectedCount` 引数で事前チェック
  - 件数不一致の場合は `scene_count_mismatch` エラーとして処理を中断し、upsert は行わない

**突合詳細ロジック**:

| ケース | 対処 |
|---|---|
| 全 record_id が一致 | 正常。ID マップで突合 |
| 不一致が全体の 20% 以下かつ件数一致 | 警告ログ + インデックス順フォールバック許容 |
| 不一致が全体の 20% 超 | fail（データ破壊防止のため upsert 禁止） |
| 全 record_id が不一致 | fail（インデックス順フォールバック禁止） |
| 件数不一致（AI ≠ 入力）| validation 段階で fail-fast（オーケストレーターに到達しない） |
...
| `subtitle_short_1` | Y | AI_OUTPUT | 字幕表示ブロック1 | 必須・空文字不可 |
| `subtitle_short_2` | Y | AI_OUTPUT | 字幕表示ブロック2 | **required・空文字可**（Fix #4: 短い scene では `""` で返す。省略不可） |
...
| `subtitle_short_2` | Y | AI_OUTPUT | scene の主要短文字幕ブロック2 | **required・空文字可**（Fix #4: 短い scene では `""` で返す。省略不可） |
...
- `narration_draft`（必須）
- `narration_tts`（必須）
- `subtitle_short_1`（必須・空文字不可）
- `subtitle_short_2`（**required・空文字可**; Fix #4 統一）
...
### 11.2 short_use=Y が 0 件の場合（Fix #6）
- Short Script Build を **SKIPPED 扱い**とする（失敗ではない）
- `100_App_Logs` に `[INFO][short_skipped] short_use=Y scenes not found (count=0)` を記録する
- `current_step` は以下の通り:
  - Full が成功済みなら `STEP_05_FULL_SCRIPT_BUILD`（変更なし）
  - Full が未実行なら更新しない（`video_format=short` のみの場合は `video_format=short` での分岐なので通常は発生しないが、念のため定義）
...
## 13. `subtitle_short_1` / `subtitle_short_2` 設計方針（Fix #4 統一）

- scene 内の**自然な表示ブロック**
- 機械的等分ではなく、意味・感情・読みやすさを優先する
- 1行あたり 12〜18文字程度を目安

### `subtitle_short_2` の統一ルール（Full / Short 共通）

| 項目 | ルール |
|---|---|
| 必須区分 | `required`（省略不可・フィールド自体を省いてはいけない） |
| 空文字 | 許容（`""` は valid）。短い scene では `""` を返すこと |
| `minLength` | なし（空文字を許容するため） |
| validator | `undefined` / `null` のみ reject。`""` は pass |

> **統一根拠**: スキーマ（`script_*_schema_ai_v1.json`）、バリデーター（`validate-json.ts`）、
> 型定義（`types.ts`）、フィールドガイド（`prompts/fragments/`）、
> サンプル（`examples/`）の全ファイルで一貫してこのルールを採用する（Fix #4）。
...
> - Full成功 / Short SKIPPED（short_use=0件）: `current_step = STEP_05_FULL_SCRIPT_BUILD`（変更なし）
> - Full失敗 / Short SKIPPED（dependency_failure）: `current_step` 更新しない
> - いずれも `100_App_Logs` に `[WARN][partial_success]` または `[INFO][short_skipped]` / `[WARN][dependency_failure]` を記録する。
> - ⚠️ `STEP_04_05_PARTIAL` とい��値は使用しない（§17 旧草案から変更）。
...
| Full schema 失敗 | Full を失敗扱い、Short は依存関係 fail でスキップ（Fix #1） | `PENDING`（partial）|
...
| short_use=Y 0 件（video_format=short/short+full） | SKIPPED 扱い（ログのみ）、失敗ではない | 更新しない（Full 成功なら既に設定済み） |
...
- `subtitle_short_2` は `required` に含める（省略不可）が `minLength` は設定しない（空文字 `""` 許容; Fix #4）
...
- `video_format=short+full` 時は Full 成功を Short 実行の前提条件とする（Fix #1）
- AI 出力件数 ≠ 入力 scenes 件数は validation で fail-fast（Fix #5）
- `short_use=Y` が 0件の場合は SKIPPED 扱い（失敗にしない）（Fix #6）
...
| ファイル | 追記内容 | ステータス |
|---|---|---|
| `src/types.ts` | ScriptFullAiRow, ScriptShortAiRow, ScriptFullRow, ScriptShortRow, ScriptFullReadRow; StepId に STEP_04/STEP_05 値を追加（Fix #2）; scene_id/scene_order の GSS 非書き込み前提を整理（Fix #3） | ✅ 完了 |
| `src/lib/validate-json.ts` | validateScriptFullAiResponse, validateScriptShortAiResponse（expectedCount fix-fast Fix #5; subtitle_short_2 undefined reject Fix #4; Ajv compile cache Fix #9; extractJson robustness Fix #9） | ✅ 完了 |
...
| `src/lib/write-app-log.ts` | buildStep04/05 SuccessLog, FailureLog; buildStep04_05PreflightFailureLog; buildStep04_05PartialSuccessLog; buildStep04ShortSkippedLog; buildStep04DependencySkippedLog（Fix #7/#8） | ✅ 完了 |
| `src/index.ts` | STEP_04_05 ルーティングケース | ✅ 完了 |
| `package.json` | build エントリ追加、dry-run:step04-05 スクリプト追加 | ✅ 完了 |

---

## 26. 実装上の補足メモ（v1.1 → v1.2 更新）
...
### 26.4 Fix #1: short+full 依存性ルール（v1.2 追記）

`video_format=short+full` 時の Short 生成は Full の成功を前提とする。
オーケストレーター内の `shortDependsOnFull` フラグで制御しており、
Full が失敗した場合は `buildStep04DependencySkippedLog` を記録して Short 実行をスキップする。
これは仕様 §4.4 step 7 に明記されている設計意図に沿った実装である。

### 26.5 Fix #5: record_id fail-fast（v1.2 追記）

`validateScriptFullAiResponse` / `validateScriptShortAiResponse` は `expectedCount` 引数を受け取り、
AI 出力件数が入力 scenes 件数と一致しない場合は `scene_count_mismatch` エラーとして早期 fail する。
オーケストレーターに到達する前にバリデーション段階でエラーを捕捉する設計となっている。

`matchAiOutputToScenes` での record_id 突合フォールバック上限:
- 不一致が全体の 20% 以下: 警告ログ + インデックス順フォールバック
- 20% 超: fail（`null` を返し、upsert をスキップ）

### 26.6 Fix #6: short_use=Y 0件 SKIPPED（v1.2 追記）

`short_use=Y` の scene が 0件の場合は失敗ではなく SKIPPED 扱いとする。
`buildStep04ShortSkippedLog` で `[INFO][short_skipped]` ログを記録する。
`current_step` は Full が成功済みなら `STEP_05_FULL_SCRIPT_BUILD` のまま維持される。

### 26.7 Fix #4: subtitle_short_2 統一（v1.2 追記）

`subtitle_short_2` は Full / Short 両方のスキーマで `required` に含めるが `minLength` は設定しない。
バリデーターは `undefined` / `null` のみ reject し、`""` は通過させる。
この統一ルールはスキーマ・バリデーター・型定義・フィールドガイド・サンプルの全ファイルで適用済み。

### 26.8 Fix #7/#8: ログビルダー整理（v1.2 追記）

`write-app-log.ts` に以下の新規ビルダーを追加した:
- `buildStep04_05PreflightFailureLog`: 前段エラー（video_format 不正・scenes 0件等）専用
- `buildStep04_05PartialSuccessLog`: partial success 専用（Full/Short 各結果を引数で受け取る）
- `buildStep04ShortSkippedLog`: short_use=Y 0件 SKIPPED 専用
- `buildStep04DependencySkippedLog`: Full 失敗による Short スキップ（dependency_failure）専用

これにより `buildStep05FailureLog` を前段エラーに流用する実装上の誤りを解消した。
