# STEP_08A TTS Subtitle & Edit Plan Build 実装設計書 v0.3

> **ステータス**: 確定（オーナー判断反映済み 2026-04-24）
> **改訂履歴**:
...
> - v0.3 (2026-04-24): tts_text 漢字仮名交じり文化・prosody rate 固定・subtitle_text 表記ルール化・voice_style prosody 明文化・subtitle_text_alt optional 化・maxOutputTokens 修正
...
- **`08_TTS_Subtitles`**: SSML 形式の TTS 音声指示テキスト（tts_text）、音声パラメータ（voice_style / speech_rate / pitch_hint / emotion_hint）、字幕テキスト（subtitle_text）
...
| 論点8 | `tts_text` | **Gemini が生成した SSML 文字列（漢字仮名交じり文）** | `narration_tts` をベースに漢字仮名交じり文に変換しつつ `pause_hint` / `emphasis_word` / `emotion_hint` を反映した `<speak>〜</speak>` SSML を Gemini が出力。`<prosody rate>` は常に `"1.0"` 固定。速度制御は TTS API の `speakingRate` のみで行う |
...
| 論点11 | `subtitle_text` 表記 | **ひらがな統一・全角80文字以内** | 漢字・カタカナ不使用。キャラクター発言は「　」で囲む。重要語前後に全角スペース挿入。`subtitle_text_alt` は常に `""` |

### `voice_style` 固定 enum と SSML 反映ルール

1物語 = 1声優（voice name 固定）。`voice_style` はシーンごとの**演技トーン**を表す。
SSML では `<prosody>` の `pitch` / `volume` 属性で差別化する（`rate` は常に `"1.0"` 固定）。

| 値 | 意味 | prosody 属性 | 使用例 |
|---|---|---|---|
| `narrator` | 標準ナレーション（落ち着いた第三者視点） | `rate="1.0"` のみ | 物語冒頭・状況説明 |
| `gentle` | やさしい・温かみのある語り口 | `rate="1.0" volume="soft"` | 穏やかな日常シーン |
| `excited` | 明るく活気のある表現 | `rate="1.0" pitch="+2st"` | 喜び・発見・勝利場面 |
| `tense` | 緊張感・ドラマティックな場面 | `rate="1.0" pitch="-1st"` | 対峙・葛藤・クライマックス |
| `whisper` | ひそやか・内緒話・思索的 | `rate="1.0" volume="x-soft"` | 噂話・独白・秘密の場面 |
| `cheerful` | 子ども向け・元気いっぱい | `rate="1.0" pitch="+3st"` | 子ども登場・歌・游び場面 |
...
| `tts_text` | AI_OUTPUT | Gemini が生成した SSML 文字列（`<speak>〜</speak>`）。**漢字仮名交じり文**。`<prosody rate="1.0">` 固定。`narration_tts` + `pause_hint` / `emphasis_word` / `emotion_hint` を素材として構築 |
...
| `subtitle_text` | AI_OUTPUT | Gemini 出力。**ひらがな統一・全角80文字以内**。キャラクター発言は「　」で囲む。重要語前後に全角スペース |
| `subtitle_text_alt` | AI_OUTPUT | **常に `""` 固定**（optional。`subtitle_text` に全文を収める） |
...
| 素材フィールド | SSML への反映 |
|---|---|
| `narration_tts` | SSML のベーステキスト（**漢字仮名交じり文に変換**して使用） |
| `pause_hint`（Full 版のみ） | `<break time="Xms"/>` に変換 |
| `emphasis_word` | `<sub>` タグによるアクセント補助 |
| `emotion_hint` | `voice_style` の `prosody` 属性選択に反映 |
| `pitch_hint` | SSML 生成意図の説明（Gemini が解釈） |

**SSML 生成の重要ルール:**
- `<prosody rate>` は常に `"1.0"` 固定（速度制御は TTS API の `speakingRate` に一本化）
- テキストは漢字仮名交じり文（narration_tts がひらがなでも変換する）
- `voice_style` に応じた `pitch` / `volume` 属性を `<prosody>` に付与する

**擬音語・擬態語・固有名詞への対応:**
- Gemini は擬音語（どんぶらこ・ざあざあ・ふわふわ 等）を検出し、
  `<sub>` タグで標準的な日本語ピッチアクセントを補助する
- 例: `<sub alias="どんぶらこ">どんぶらこ</sub>`
...
| `maxOutputTokens` | `32768`（24シーン分の SSML 出力が約 20,000〜25,000 トークンになるため） |
 ...
 *
 * tts_text: narration_tts を漢字仮名交じり文に変換しつつ Gemini が生成した SSML。
 *   <prosody rate> は常に "1.0" 固定。速度制御は TTS API の speakingRate に一本化。
 * subtitle_text: ひらがな統一・全角80文字以内・キャラクター発言は「」囲み。
 * subtitle_text_alt: optional。常に "" を推奨（subtitle_text に全文を収める）。
 */
export interface TtsSubtitleAiRow {
  scene_record_id:    string;   // 照合用: Script.record_id（GSS 書き込み対象外）
  tts_text:           string;   // SSML 文字列（<speak>〜</speak>・漢字仮名交じり文）
  ...
  subtitle_text:      string;   // 主字幕テキスト（ひらがな・全角80文字以内）
...
その他の型定義（TtsSubtitleRow / TtsSubtitleReadRow / EditPlanAiRow / EditPlanRow / EditPlanReadRow）は v0.2 から変更なし。

---

## 12. モジュール設計

v0.2 から変更なし。詳細は `step08a_implementation_spec_v0.2.md` §12 を参照。

---

## 13. INPUT_DATA 仕様（プロンプト注入 JSON）

v0.2 から変更なし。詳細は `step08a_implementation_spec_v0.2.md` §13 を参照。

---

## 14. 100_App_Logs Upsert 仕様

v0.2 から変更なし。詳細は `step08a_implementation_spec_v0.2.md` §14 を参照。

---

## 15. オーケストレーター設計詳細

v0.2 から変更なし。詳細は `step08a_implementation_spec_v0.2.md` §15 を参照。
