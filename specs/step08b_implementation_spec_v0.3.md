...
**⑤`<sub>` タグ間の句読点を `<break>` に置換（v0.3.1 追加）**
`<sub>` タグ直後の `、` `。` は TTS に「テン」「マル」と読まれる。
`<break>` タグに置換することで正しく「間」として処理される。

```xml
<!-- ❌ NG: "どんぶらこ・テン・どんぶらこ" と読まれる -->
...
| 対象 | 置換後 |
|---|---|
| `</sub>、` | `</sub><break time="200ms"/>` |
| `</sub>。` | `</sub><break time="300ms"/>` |
