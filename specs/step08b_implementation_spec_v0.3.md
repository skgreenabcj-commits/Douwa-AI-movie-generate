...
**⑤`</sub>` 直後の句読点を `<break>` に置換（v0.3.1/v0.3.2 更新）**
`</sub>` 直後の `、` `。` は、後続が別の `<sub>` でも通常テキストでも
TTS が音声アーティファクト（"テン" 読み・"あぁ" 息継ぎ音 等）を生成する。
**後続種別に関わらず `</sub>` 直後の句読点はすべて `<break>` に置換する。**

```xml
<!-- ❌ NG: </sub> + 通常テキスト → "あぁ" 等のアーティファクト（v0.3.2 追加ケース） -->
猿が<sub alias="ひょっこり">ひょっこり</sub>、顔を出しました

<!-- ❌ NG: </sub> + <sub> → "テン" と読まれる -->
<sub alias="どんぶらこ">どんぶらこ</sub>、<sub alias="どんぶらこ">どんぶらこ</sub>と、

<!-- ✅ OK: どちらも </sub> 直後は <break> -->
