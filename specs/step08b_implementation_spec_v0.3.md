...
> **v0.2 からの変更**: `rate` を prosody から排除。SSML 内 `<prosody rate>` は常に `"1.0"` 固定。
> `gentle` は `rate="slow" volume="soft"` → `rate="1.0" volume="soft"` へ変更。
> `excited` / `whisper` から `rate` 属性を削除。
...
```xml
<!-- excited の例 -->
<speak><prosody rate="1.0" pitch="+2st">鬼を倒しました！</prosody></speak>
