...
## RTK Token Optimization（Git コマンド）

RTK is globally enabled via `rtk init -g`. This project uses RTK for Git operations:

### Git Commands Compression
- `git status` → compact format (e.g., 3 modified, 1 untracked)
- `git log -n N` → summary format (commit hash + message only)
- `git diff` → file-level stats only
- `git diff --stat` → file changes summary

### Expected Token Savings
- Git operations: 80-92% reduction

### Verification
To verify token savings, run:
rtk gain

### Notes
- RTK is integrated at the PreToolUse hook level for git commands only
- Other commands (npm, build, typecheck) continue with existing PostToolUse hooks
- dist/ edits remain blocked by the existing PostToolUse hook
